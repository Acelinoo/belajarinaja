  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto bg-[#FCFBF8] text-[#1e293b] shadow-2xl rounded-xl overflow-hidden border border-slate-300 print:border-none print:shadow-none print:rounded-none print:max-w-none print:w-full select-none">
      ...
      <div className="p-1.5 sm:p-5">
        <div className="relative border-2 border-[#1e293b] p-3.5 sm:p-10 lg:p-14 bg-[#FFFFFF] flex flex-col justify-between min-h-[520px] sm:min-h-[660px]">
          {/* INNER FINE GOLD BORDER */}
          <div className="absolute inset-1.5 sm:inset-3 border border-[#c5a059]/70 pointer-events-none" />

          {/* CORNER ORNAMENTS (4 CORNERS) */}
          {/* Top-Left */}
          <svg
            className="absolute top-1.5 left-1.5 sm:top-3 sm:left-3 w-5 h-5 sm:w-7 sm:h-7 text-[#c5a059] pointer-events-none"
            viewBox="0 0 40 40"
            fill="currentColor"
          >
          ...
          {/* Top-Right */}
          <svg
            className="absolute top-1.5 right-1.5 sm:top-3 sm:right-3 w-5 h-5 sm:w-7 sm:h-7 text-[#c5a059] pointer-events-none"
            viewBox="0 0 40 40"
            fill="currentColor"
          >
          ...
          {/* Bottom-Left */}
          <svg
            className="absolute bottom-1.5 left-1.5 sm:bottom-3 sm:left-3 w-5 h-5 sm:w-7 sm:h-7 text-[#c5a059] pointer-events-none"
            viewBox="0 0 40 40"
            fill="currentColor"
          >
          ...
          {/* Bottom-Right */}
          <svg
            className="absolute bottom-1.5 right-1.5 sm:bottom-3 sm:right-3 w-5 h-5 sm:w-7 sm:h-7 text-[#c5a059] pointer-events-none"
            viewBox="0 0 40 40"
            fill="currentColor"
          >
          ...
          <div className="relative text-center space-y-1.5 pb-4 border-b border-slate-200/80">
            <div className="flex items-center justify-center gap-2.5 sm:gap-3">
              ...
              <div className="text-left">
                <span className="font-serif font-bold text-base sm:text-xl tracking-wide text-[#0f172a] block leading-tight">
                  BELAJARINAJA
                </span>
                <span className="text-[9px] sm:text-[10px] font-mono tracking-[0.15em] sm:tracking-[0.2em] text-[#64748b] uppercase block">
                  ACADEMIC SYSTEM OF SOFTWARE ENGINEERING
                </span>
              </div>
            </div>
            <p className="text-[9px] sm:text-[10px] tracking-[0.15em] sm:tracking-[0.25em] text-[#c5a059] uppercase font-serif font-semibold">
              DEWAN AKREDITASI & STANDARISASI KOMPETENSI REKAYASA WEB
            </p>
          </div>

          {/* 2. MAIN TITLE SECTION */}
          <div className="relative text-center space-y-2 sm:space-y-3 my-3">
            <h1 className="font-serif text-xl sm:text-4xl font-bold tracking-[0.12em] sm:tracking-[0.18em] text-[#0f172a] uppercase">
              {language === "en" ? "Certificate of Completion" : "Sertifikat Kelulusan"}
            </h1>

            {/* Ornamental Divider */}
            <div className="flex items-center justify-center gap-2 sm:gap-3">
              <div className="w-12 sm:w-28 h-px bg-gradient-to-r from-transparent to-[#c5a059]" />
              <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rotate-45 bg-[#c5a059]" />
              <div className="w-12 sm:w-28 h-px bg-gradient-to-l from-transparent to-[#c5a059]" />
            </div>
          ...
          {/* 3. RECIPIENT NAME */}
          <div className="relative text-center space-y-1.5 my-2">
            <div className="font-serif text-xl sm:text-3xl lg:text-5xl font-bold text-[#0f172a] tracking-wide capitalize sm:uppercase break-words px-2">
              {studentName}
            </div>
            <div className="w-36 sm:w-72 h-0.5 bg-[#c5a059] mx-auto opacity-80" />
            <span className="text-[10px] sm:text-[11px] font-mono text-[#64748b] block">
              Student ID: @{studentUsername}
            </span>
          </div>

          {/* 4. FORMAL CITATION / COURSE DETAILS */}
          <div className="relative text-center max-w-2xl mx-auto space-y-1.5 my-2 px-2">
            <p className="text-[11px] sm:text-[13px] leading-relaxed text-[#334155] font-serif">
              {language === "en" ? (
                <>
            ...
            <h2 className="font-serif font-extrabold text-sm sm:text-lg tracking-wider text-[#0f172a] uppercase py-0.5 sm:py-1 break-words">
              Modern Fullstack Web Development & Software Engineering
            </h2>

            <p className="text-[10px] sm:text-xs leading-relaxed text-[#64748b] font-normal">
              {language === "en" ? (
                <>
          ...
          <div className="relative pt-5 sm:pt-8 border-t border-slate-200/80 flex flex-col sm:flex-row items-center justify-between gap-5 sm:gap-8 max-w-2xl mx-auto w-full px-2 sm:px-12 my-2">
            {/* Left Digital Signature: Founder */}
            <div className="text-center flex flex-col items-center">
              <svg
                className="h-9 sm:h-11 w-28 sm:w-32 text-[#0f172a]"
                viewBox="0 0 120 40"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              ...
              <div className="w-32 sm:w-36 border-b border-[#0f172a] mt-1 mb-1.5" />
              <span className="font-serif font-bold text-xs sm:text-sm text-[#0f172a] block">
                Marchelino Kurniawan
              </span>
              <span className="text-[9.5px] sm:text-[10px] font-sans text-[#64748b] block">
                Founder & Fullstack Specialist
              </span>
              <span className="text-[8px] sm:text-[8.5px] font-mono text-emerald-600 font-semibold mt-0.5 tracking-wider uppercase block">
          ...
          <div className="relative pt-3 sm:pt-4 border-t border-slate-200 text-[9px] sm:text-[10px] font-mono text-[#64748b] flex flex-col sm:flex-row items-center justify-between gap-1.5 sm:gap-2">
            <div>
            ...
            <div className="text-center max-w-full truncate px-1">
