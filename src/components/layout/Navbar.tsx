    ...
    <header className="sticky top-0 z-40 w-full border-b border-border bg-card/95 backdrop-blur-md transition-colors">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-14 sm:h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {isAuthenticated ? (
            <Link href="/dashboard">
              <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-secondary text-xs font-semibold text-foreground border border-border">
            ...
            <Button
              ...
              className="h-8 text-xs font-semibold px-3.5 rounded-md"
            >
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-border bg-card p-4 space-y-1 animate-in slide-in-from-top-2">
          {navLinks.map((link) => {
            const isActive = pathname === link.href;
            const Icon = link.icon;
            return (
